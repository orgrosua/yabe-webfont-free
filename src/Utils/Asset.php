...
class Asset
{
    ...
    {
        if (static::$manifest === []) {
            $manifest = \file_get_contents(\dirname(YABE_WEBFONT::FILE) . '/build/manifest.json');
    ...
    public static function read_entrypoints() : array
    {
        if (static::$entrypoints === []) {
            $entrypoints = \file_get_contents(\dirname(YABE_WEBFONT::FILE) . '/build/entrypoints.json');
        ...
        if (isset($entrypoints[$key])) {
            ...
            if (isset($entry['js'])) {
                foreach ($entry['js'] as $js) {
                    $manifest_key = \array_search($js, $manifest, \true);
                     ...
                     */
                    $dependency = \apply_filters('f!yabe/webfont/asset:enqueue_entry.dependency', $dependency, $manifest_key, $key);
                    static::enqueue_script($manifest_key, $dependency, '', $in_footer);
                }
            }
            if (isset($entry['css'])) {
                foreach ($entry['css'] as $css) {
                    $manifest_key = \array_search($css, $manifest, \true);
                    $dependency = $deps[$manifest_key] ?? [];
                    /**
                     * Filter the dependency for the manifest key.
                     *
    ...
    public static function register_script(string $key, array $deps = [], string $src = '', bool $in_footer = \false)
    {
        ...
        if ($src === '') {
            ...
            if (isset($manifest[$key])) {
                $src = \strncmp($manifest[$key], 'http', \strlen('http')) === 0 ? $manifest[$key] : \plugins_url('build/' . $manifest[$key], YABE_WEBFONT::FILE);
            } else {
                return \false;
            }
        }
    ...
    public static function register_style(string $key, array $deps = [], string $src = '', string $media = 'all')
    {
        ...
        $is_registered = \wp_register_style($handle, $src, $deps, YABE_WEBFONT::VERSION, $media);
        if ($is_registered) {
            return $handle;
        }
        return \false;
