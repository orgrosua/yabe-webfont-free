...
final class Plugin
{
        ...
        \add_action('admin_notices', static function () {
            ...
            if ($messages && \is_array($messages)) {
                foreach ($messages as $message) {
                    echo \sprintf('<div class="notice notice-%s is-dismissible %s">%s</div>', \esc_attr($message['status']), \YABE_WEBFONT_OPTION_NAMESPACE, \esc_html($message['message']));
                }
            }
        }, 100);
    }
    ...
    public function maybe_update_plugin()
    {
        ...
        $this->plugin_updater = new PluginUpdater(\YABE_WEBFONT_OPTION_NAMESPACE, ['version' => self::VERSION, 'license' => $license['key'] ? \trim($license['key']) : \false, 'beta' => $license['opt_in_pre_release'], 'plugin_file' => \YABE_WEBFONT_FILE, 'item_id' => \YABE_WEBFONT_EDD_STORE['item_id'], 'store_url' => \YABE_WEBFONT_EDD_STORE['url'], 'author' => \YABE_WEBFONT_EDD_STORE['author']]);
        return $this->plugin_updater;
    }
    /**
    ...
    private function maybe_embedded_license() : void
    {
        ...
        \update_option(\YABE_WEBFONT_OPTION_NAMESPACE . '_license', ['key' => $license_key, 'opt_in_pre_release' => \false]);
        \unlink($license_file);
